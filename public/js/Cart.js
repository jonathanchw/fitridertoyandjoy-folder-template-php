...
function addToCart(product) {
    ...
    let itemExists = cartData.find((prd) => prd.id === product.id);
    if (!itemExists) {
        product.quantity = 1;
...
function handleCartItems() {
    ...
    cartTable.innerHTML += cartData
        .map(({ id, name, images, price, shipping_price }) => {
            let shipping = shipping_price !== null ? shipping_price : 0;
            let parsedPrice = parseFloat(price).toFixed(2); // Convert to a string with 2 decimal places
            let parsedShipping = parseFloat(shipping).toFixed(2); // Convert to a string with 2 decimal places

            let totalPrice = (
                parseFloat(parsedPrice) + parseFloat(parsedShipping)
            ).toFixed(2);
            return `
  ...
  <td id="total-${id} class="priceCol">$${totalPrice}</td>
  </tr>
  `;
        })
        .join("");
